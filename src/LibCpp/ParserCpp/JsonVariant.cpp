...
boost::variant<std::string, json_array *> &JsonVariant::get_value()
{
    return value;
};
...
const std::string &JsonVariant::operator[](const std::string &key)
{
    const std::string &s = boost::get<std::string>(value);
    return (s);
}

json_pair JsonVariant::operator[](int key)
{
    try {
